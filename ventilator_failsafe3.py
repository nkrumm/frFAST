...
if source_filename[-3:].lower() == 'bam':
    ...
    try:
        ...
        errorMsg("ERROR VENT cannot open BAM file (io error): " + source_filename)
    except ValueError:
        errorMsg("ERROR VENT cannot open BAM file (invalid bam file?): " + source_filename)
...
while not endOfFile:
    ...
    connected = False
    for n_tries in range(0,MAX_TRIES):
        while not connected:
            try:
                ...
                # port already in use!
                print readstreamer
                #errorMsg("WARNING VENT mapper readstream port not available! Attempting to retry %d of %d tries" % (n_tries, MAX_TRIES),quit=False)
                print "WARNING VENT mapper readstream port not available! Attempting to retry %d of %d tries" % (n_tries, MAX_TRIES)
                time.sleep(10)
                continue
            connected = True
        break
    ...
    while read_counter < maximum_reads_per_destination:
        ...
        if read.is_reverse:
            if read.rlen == 36:
                ...
                if read.rlen < 72:
                    readstreamer.send("%s %s" % (read.qname, reverseComplement(read.seq[read.rlen-36:])))
                    read_counter += 1
                else:               
