...
rc_offset_start = read_length - (split_length * number_splits)
	...
	if number_splits == 1:
		while read_counter < maximum_reads_per_destination:
			try:
				...
				endOfFile = True
				break
			if read.is_reverse: #read[1] == '83' or read[1] == '147':# needs to be reverse complemented.
				readstreamer.send("%s %s" % (read.qname, reverseComplement(read.seq[rc_offset_start:])))
			else:
				readstreamer.send("%s %s" % (read.qname,read.seq[0:split_length]))
			read_counter += 1
	elif number_splits == 2:
		end_split = split_length * 2
		while read_counter < maximum_reads_per_destination:
			try:
				read = samfile.next()
			except StopIteration:
				...
				break
			if read.is_reverse: #read[1] == '83' or read[1] == '147':# needs to be reverse complemented.
				seq = reverseComplement(read.seq[rc_offset_start:])
		...
		# Not implemented
		requester.send("ERROR VENT greater than two splits not yet supported!: " + str(number_splits))
		_ = requester.recv()
		exit(0)
