...
while 1: 
	...
	try:
		mapperID, readname, chr, position, edit = data_msg.split(" ")
	...
	chr = int(chr[3:])
	if chr not in (23,24,53,54):
		try:
			autosomal_readnames[readname] += 1
		except KeyError:
			autosomal_readnames[readname] = 1
	
	mapped_mapperID[counter] = int(mapperID)
	mapped_chrs[counter] = chr
...
print "autosomal_readnames: " + str(len(autosomal_readnames))
