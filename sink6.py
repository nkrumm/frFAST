...
try:
	pubsub = context.socket(zmq.SUB)
	pubsub.connect("tcp://%s:%d" % (controlleraddress, PUB_SUB_PORT))
	pubsub.setsockopt(zmq.SUBSCRIBE, "SINK")
except zmq.core.error.ZMQError:
	# port already in use!
	errorMsg("ERROR SINK pub_sub port not available!")

poller = zmq.Poller()
poller.register(requester, zmq.POLLIN)
poller.register(requester, zmq.POLLOUT)
poller.register(pubsub, zmq.POLLIN)
poller.register(insock, zmq.POLLIN)


while 1: 
	socks = dict(poller.poll(-1))
	if socks.get(pubsub) == zmq.POLLIN:
		data_msg = pubsub.recv()
		if data_msg == 'SINK KILL':
			# we expect this signal to only be sent when vent_is_done is set in controller and mappers.NumWorking is == 0, indicating that all the mappers had finished sending their reads to the sink
			# therefore we can break out of the loop.
			print "SINK KILL RECEIVED"
			pubsub.close()
			break
	if socks.get(insock) == zmq.POLLIN:
		...
		if counter == MAX_READS:
			print "SINK UPDATE " + str(counter)
			sys.stdout.flush()
			requester.send("SINK UPDATE " + str(counter))
			#socks = dict(poller.poll())
			_ = requester.recv()
			socks = dict(poller.poll())
			print "SINK UPDATE DONE" + str(counter)
			sys.stdout.flush()
